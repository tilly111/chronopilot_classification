...
from utils.splitting import only_one_minute
...
if __name__ == '__main__':
    ...
    # for ind_idx in splits:

    # print(f"train: {train_idx}")
    # print(f"test: {test_idx}")
    for which_minute in range(1, 6):
        if n_classes == 2:
        ...
        idx = only_one_minute(X, y, "slice", minute=which_minute)

        print("\n")
        print("--------------------------------------------------")
        # print(f"train on {y.iloc[train_idx]['time'].unique()}")
        print(f"test on {y.iloc[idx]['time'].unique()}")
        print(f"test on {y.iloc[idx]['participant'].unique()}")
        print(f"test on {y.iloc[idx]['robot'].unique()}")
        # print("--------------------------------------------------")
        # X_train = X.iloc[train_idx].drop(columns=["slice", "slice_fix", "participant", "time", "robot"])
        X_individual_min = X.iloc[idx].drop(columns=["slice", "participant", "time", "robot"])
        # y_train = y.iloc[train_idx].drop(columns=["participant", "time", "robot"])
        y_individual_min = y.iloc[idx].drop(columns=["participant", "time", "robot"])
        ...
        conf_m_result_frame.to_csv(
            f"results/eye_tracking_{n_classes}_classes/only_one_minute/confusion_matrix_{label}_tw_{tw}_minute_{which_minute}{tag}.csv",
            index=False)

        accuracy_result_frame.loc[which_minute] = acc_list

    accuracy_result_frame.index.name = "only_one_minute"
    print(accuracy_result_frame)
    accuracy_result_frame.to_csv(f"results/eye_tracking_{n_classes}_classes/only_one_minute/accuracy_{label}_tw_{tw}{tag}.csv")
