...
if __name__ == '__main__':
    ...
    for train_idx, test_idx in splits:
        participant = int(y.iloc[test_idx]['participant'].unique()[0])
        if n_classes == 2:
        ...
        # print("--------------------------------------------------")
        X_train = X.iloc[train_idx].drop(columns=["slice", "participant", "time", "robot"])
        X_test = X.iloc[test_idx].drop(columns=["slice", "participant", "time", "robot"])
        y_train = y.iloc[train_idx].drop(columns=["participant", "time", "robot"])
        y_test = y.iloc[test_idx].drop(columns=["participant", "time", "robot"])
        ...
        # TODO logg results
        accuracy_result_frame.loc[participant] = acc_list

        conf_m_result_frame.to_csv(
            f"results/eye_tracking_{n_classes}_classes/participants/confusion_matrix_{label}_tw_{tw}_participant_{participant}{tag}.csv",
            index=False)

    accuracy_result_frame.index.name = "participant"
    print(accuracy_result_frame)
    accuracy_result_frame.to_csv(f"results/eye_tracking_{n_classes}_classes/participants/accuracy_{label}_tw_{tw}{tag}.csv")
