...
def fit_classifier_parallel(X, y, pl_interpretable, i_train, i_validation, n_classes):
    ...
    return accuracy_score(y_validation, y_pred), roc, f1_score(y_validation, y_pred, average='weighted'), trained
...
if __name__ == '__main__':
    ...
    x_analysis = X[((X["participant"] != participant_id) | (X["slice"] < init_length / tw))].copy()
    y_analysis = y[((y["participant"] != participant_id) | (X["slice"] < init_length / tw))].copy()
    x_test = X[((X["participant"] == participant_id) & (X["slice"] >= init_length / tw))].copy()
    y_test = y[((y["participant"] == participant_id) & (X["slice"] >= init_length / tw))].copy()
    ...
    save_frame.to_csv(
        f"results/eye_tracking_{n_classes}_classes/participants/{scoring}_tw_{tw}_label_{label}{tag}_{participant_id}.csv")
