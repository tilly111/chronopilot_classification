...
if __name__ == '__main__':
    ...
    for _, ind_idx in splits:
        ...
        robot = int(y.iloc[ind_idx]['robot'].unique()[0])

        print("\n")
        print("--------------------------------------------------")
        # print(f"train on {y.iloc[train_idx]['time'].unique()}")
        # print(f"test on {y.iloc[ind_idx]['robot'].unique()}")
        # print("--------------------------------------------------")
        ...
        with ProcessPoolExecutor(max_workers=m_workers) as executor:
            for seed in range(num_splits):
                X_train, X_test, y_train, y_test = train_test_split(X_individual_min, y_individual_min,
                                                                    ...
                                                                    random_state=seed)
                print(f"y_test {y_test.shape}, unique {np.unique(y_test, return_counts=True)}")
                futures.append(
                    executor.submit(
        ...
        if save_results:
            conf_m_result_frame.to_csv(
                f"results/eye_tracking_{n_classes}_classes/robots/confusion_matrix_{label}_tw_{tw}_robot_{robot}{tag}.csv",
                index=False)

        accuracy_result_frame.loc[robot] = acc_list

    accuracy_result_frame.index.name = "robot"
    print(accuracy_result_frame)
    if save_results:
        accuracy_result_frame.to_csv(f"results/eye_tracking_{n_classes}_classes/robots/accuracy_{label}_tw_{tw}{tag}.csv")
