...
if slicing == "minute":
    ...
    X, y = load_eye_tracking_data_tw(number_of_classes=n_classes, load_preprocessed=False, tw=tw, label_name=[label])
    X.drop(columns=["slice", "slice_fix"], inplace=True)
else:
...
if slicing == "minute":
    ...
    x_train.to_csv(f"preprocessed_data/eye_tracking_{n_classes}_classes/X_train_tw_{tw}_label_{label}.csv", index=False)
    x_test.to_csv(f"preprocessed_data/eye_tracking_{n_classes}_classes/X_test_tw_{tw}_label_{label}.csv", index=False)
    y_train.to_csv(f"preprocessed_data/eye_tracking_{n_classes}_classes/y_train_tw_{tw}_label_{label}.csv", index=False)
    y_test.to_csv(f"preprocessed_data/eye_tracking_{n_classes}_classes/y_test_tw_{tw}_label_{label}.csv", index=False)
else:
