...
from classifier.ffn import FeedForwardNN
...
manual_features = ["breathingrate", "bpm", "hr_mad"]
tmp_all_acc = []
...
for p_t in ksubsets(population, 1):  # 3
    ...
    if feature_selection == "RFECV":
        selector = RFECV(estimator, step=1, cv=len(p_train), min_features_to_select=1)
