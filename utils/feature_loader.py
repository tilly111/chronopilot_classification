...
def load_eye_tracking_data_tw(number_of_classes=2, load_preprocessed=True, tw=10, label_name=["ppot"],
    ...
    if bls:
        ...
        tag = ""
    df_pupil = pd.read_csv(f"/Volumes/Data/chronopilot/Julia_study/features/all_exp_pupil_features_tw_{tw}{tag}.csv")
    df_fixation = pd.read_csv(f"/Volumes/Data/chronopilot/Julia_study/features/all_exp_fixations_features_tw_{tw}{tag}.csv")
            ...
            if not np.isnan(y).any():
                ...
                if number_of_classes == 2:
                    if label_name[0] == "ppot":
                        ...
                        y[0, 0] = 1 if y[0, 0] >= 0.9 else 0 # TODO adjust for slight underestimation in most settings -> 0.9 almost balanced classes
                elif number_of_classes == 3:
                    if label_name[0] == "ppot":
                        if y[0,0] < 2:
                            y[0,0] = 0
...
def _load_eye_tracking_data_tw_preprocessed(number_of_classes, tw, label_name, include_meta_label, load_test, bls=False) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if load_test:
        ...
        if number_of_classes == 2:
            X_test = pd.read_csv(f"preprocessed_data/eye_tracking_2_classes/X_test_tw_{tw}_label_{label_name[0]}{tag}.csv")
            y_test = pd.read_csv(f"preprocessed_data/eye_tracking_2_classes/y_test_tw_{tw}_label_{label_name[0]}{tag}.csv")
        elif number_of_classes == 3:
            X_test = pd.read_csv(f"preprocessed_data/eye_tracking_3_classes/X_test_tw_{tw}_label_{label_name[0]}{tag}.csv")
            y_test = pd.read_csv(f"preprocessed_data/eye_tracking_3_classes/y_test_tw_{tw}_label_{label_name[0]}{tag}.csv")
        else:
            print("Number of classes not preprocessed")
            X_test = None
            y_test = None
        ...
        if number_of_classes == 2:
            X_train = pd.read_csv(f"preprocessed_data/eye_tracking_2_classes/X_train_tw_{tw}_label_{label_name[0]}{tag}.csv")
            y_train = pd.read_csv(f"preprocessed_data/eye_tracking_2_classes/y_train_tw_{tw}_label_{label_name[0]}{tag}.csv")
        elif number_of_classes == 3:
            X_train = pd.read_csv(f"preprocessed_data/eye_tracking_3_classes/X_train_tw_{tw}_label_{label_name[0]}{tag}.csv")
            y_train = pd.read_csv(f"preprocessed_data/eye_tracking_3_classes/y_train_tw_{tw}_label_{label_name[0]}{tag}.csv")
        else:
            print("Number of classes not preprocessed")
            X_train = None
            y_train = None
